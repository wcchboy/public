...
import com.wcch.colorpicker.view.ColorWheelSelectorByImage;
...
public class ColorWheelView extends FrameLayout implements ColorObservable, Updatable {
    ...
    //private ColorWheelSelector selector;//色轮选择器 绘制的线条一个瞄准镜的样子
    private ColorWheelSelectorByImage selector;//色轮选择器 选择为图片
    ...
    public ColorWheelView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...

        {
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
            //色轮选择器 自定义图片
            selector = new ColorWheelSelectorByImage(context);
            selector.setIconSize((int)selectorRadiusPx*2);
            //色轮选择器 绘制的线条
